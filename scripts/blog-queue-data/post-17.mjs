export default {
  ...
  description:
    "Staff-level permission models for multi-tenant AI copilots: tenant isolation, tool-level authz, retrieval filters, delegated credentials, and audit trails that survive agent fan-out.",
  readingMinutes: 7,
  keywords: [
    "multi-tenant AI",
  ...
  locales: {
    tr: {
      ...
      description:
        "Çok kiracılı AI copilottlar için staff seviyesi izin modelleri: tenant izolasyonu, tool-level authz, retrieval filtreleri, delegasyon kimlik bilgileri ve ajan fan-out'unda ayakta kalan denetim izleri.",
      sections: [
        {
    ...
    de: {
      ...
      description:
        "Staff-Level Permission Models für Multi-Tenant-AI-Copilots: Tenant-Isolation, Tool-Level-Authz, Retrieval-Filter, delegierte Credentials und Audit-Trails die Agent-Fan-out überleben.",
      sections: [
        {
    ...
    fr: {
      ...
      description:
        "Modèles de permissions staff pour copilotes IA multi-tenant : isolation tenant, authz au niveau outil, filtres de retrieval, credentials délégués et pistes d'audit qui survivent au fan-out d'agents.",
      sections: [
        {
    ...
    it: {
      ...
      description:
        "Modelli di permesso staff per copiloti AI multi-tenant: isolamento tenant, authz a livello tool, filtri di retrieval, credenziali delegate e audit trail che sopravvivono al fan-out degli agent.",
      sections: [
        {
    ...
    zh: {
      ...
      excerpt:
        "继承聊天权限却忽略数据 ACL 的多租户 AI 副驾驶会自信地引用错误租户。工具调用需要与你们的 API 走同一条授权路径——按租户、角色与资源限定范围。",
      description:
        "多租户 AI 副驾驶的 staff 级权限模型：租户隔离、工具级授权、检索过滤、委托凭证，以及能撑过智能体扇出的审计轨迹。",
      sections: [
        {
          heading: "聊天访问不等于数据访问",
          paragraphs: [
    ...
    ja: {
      title: "マルチテナントAIコパイロットの権限モデル",
      excerpt:
        "チャット権限を継承しつつデータACLを無視するマルチテナントAIコパイロットは、誤ったテナントを自信満々に引用します。ツール呼び出しはAPIと同じ認可パスを使うべきです—テナント・役割・リソースごとにスコープ。",
      description:
        "マルチテナントAIコパイロット向けスタッフ級権限モデル：テナント分離、ツール級認可、検索フィルタ、委任クレデンシャル、エージェントのファンアウトでも耐える監査証跡。",
      sections: [
        {
          heading: "チャットアクセスはデータアクセスではない",
          paragraphs: [
        ...
        {
          heading: "エージェントが代理するプリンシパルをモデル化する",
          paragraphs: [
