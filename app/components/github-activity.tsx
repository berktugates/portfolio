...
export function GitHubActivity() {
  ...
  return <div className="mt-8 overflow-hidden rounded-2xl bg-zinc-300/30 p-px dark:bg-zinc-600/30">
    <div className="rounded-[15px] bg-white p-4 dark:bg-zinc-950">
      ...
      <div className="overflow-x-auto pb-1 text-zinc-500 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"><GitHubCalendar username="berktugates" blockSize={9} blockMargin={3} fontSize={11} theme={{ light: ["#f6f8fa", "#9be9a8", "#40c463", "#30a14e", "#216e39"], dark: ["#0d1117", "#0e4429", "#006d32", "#26a641", "#39d353"] }} tooltips={{ activity: { withArrow: true, hoverRestMs: 80, text: (activity) => { const count = activity.count === 0 ? "No contributions" : `${activity.count} contribution${activity.count === 1 ? "" : "s"}`; return `${count} on ${contributionDateFormatter.format(new Date(`${activity.date}T00:00:00Z`))}`; } } }} /></div>
